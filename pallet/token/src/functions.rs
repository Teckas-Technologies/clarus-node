...
impl<T: Config> Pallet<T> {
	...
	pub(super) fn _transfer(
		...
		ensure!(<BalanceOf<T>>::contains_key(id, from.clone()), Error::<T>::AccountDoesNotOwnThisToken);
		...
		Asset::<T>::try_mutate(id, |maybe_details| -> DispatchResult {
			...
			<BalanceOf<T>>::try_mutate_exists(id, &from, |maybe_account| -> DispatchResult {
				...
				*maybe_account = if balance.is_zero() {
					details.accounts = details.accounts.checked_sub(1).ok_or(Error::<T>::AccountsUnderflow)?;
					None
				} else {
					Some(balance)
				};
		...
		Self::deposit_event(Event::Transferred { asset_id: id, from, to, amount });
		Ok(())
	}

	pub fn _transfer_all(id: T::AssetId, from: T::AccountId, to: T::AccountId) -> Result<(), DispatchError> {
		if <BalanceOf<T>>::contains_key(id, &from) {
			Asset::<T>::try_mutate(id, |maybe_details| -> DispatchResult {
				...
				<BalanceOf<T>>::try_mutate_exists(id, &from, |maybe_account| -> DispatchResult {
					...
					<BalanceOf<T>>::try_mutate(id, &to, |t| -> DispatchResult {
						let new_balance = t.checked_add(&balance).ok_or(Error::<T>::TransferOverFlow)?;
						if t.is_zero() {
							Self::new_account(&to, details)?;
						}
						*t = new_balance;
					...
					Self::deposit_event(Event::Transferred { asset_id: id, from: from.clone(), to, amount: balance });

					// Make the debit.
					balance = balance.checked_sub(&balance).ok_or(Error::<T>::TransferUnderFlow)?;
					*maybe_account = if balance.is_zero() {
						details.accounts = details.accounts.checked_sub(1).ok_or(Error::<T>::AccountsUnderflow)?;
						None
					} else {
	...
	pub(super) fn do_mint(
		...
		Self::increase_balance(id, beneficiary, amount, |details| -> DispatchResult {
			...
			details.supply = details.supply.checked_add(&amount).ok_or(Error::<T>::MintOverFlow)?;
			Ok(())
		})?;
	...
	pub(super) fn new_account(_who: &T::AccountId, d: &mut AssetDetails<T>) -> Result<(), DispatchError> {
		let accounts = d.accounts.checked_add(1).ok_or(Error::<T>::AccountsOverflow)?;
		d.accounts = accounts;
		Ok(())
	}
	...
	pub(super) fn do_burn(
		...
		let actual = Self::decrease_balance(id, target, amount, |actual, details| {
			...
			details.supply = details.supply.checked_sub(&actual).ok_or(Error::<T>::BurnUnderflow)?;
	...
	pub(super) fn decrease_balance(
		...
		Asset::<T>::try_mutate(id, |maybe_details| -> DispatchResult {
			...
			<BalanceOf<T>>::try_mutate_exists(id, target, |maybe_account| -> DispatchResult {
				...
				// Make the debit.
				balance = balance.checked_sub(&amount).ok_or(Error::<T>::BalanceUnderflow)?;
				*maybe_account = if balance.is_zero() {
					details.accounts = details.accounts.checked_sub(1).ok_or(Error::<T>::AccountsUnderflow)?;
					None
				} else {
