...
pub mod pallet {
	...
	/// Details of an asset.
	pub type Asset<T: Config> = StorageMap<_, Blake2_128Concat, T::AssetId, AssetDetails<T>, OptionQuery, GetDefault>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create(
			...
			ensure!(T::CallbackHandle::created(&id, &owner).is_ok(), Error::<T>::CallbackFailed);
		...
		pub fn approve(
			...
			Self::deposit_event(Event::Approval { asset_id: token_id, owner: sender, spender, amount: value });
	...
	pub struct GenesisConfig<T: Config> {
		pub asset: Vec<(T::AssetId, (T::AccountId, T::Balance, u32, Vec<u8>, Vec<u8>, T::Balance, u32))>,
	...
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			Self { asset: vec![], balances: vec![], init__erc20_token: false }
		}
	}
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			for (asset_id, (issuer, supply, accounts, name, symbol, total_supply, decimals)) in self.asset.iter() {

				let asset_details: AssetDetails<T> =
					AssetDetails { name: name.to_vec(), symbol: symbol.to_vec(), issuer: issuer.clone(), supply: *supply, accounts: *accounts };
